...
import { GitBranch, FileText, Link, BrainCircuit, Image, Sparkles, ArrowRight, FileJson } from 'lucide-react';
...
const Home: React.FC<HomeProps> = ({ onNavigate, onOpenExport, historyCount = 0 }) => {
  return (
    <div className="max-w-4xl mx-auto space-y-20 mb-20">
      ...
      <div className="text-center space-y-6 pt-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        <div className="flex flex-col items-center gap-6 pt-8 w-full max-w-[480px] mx-auto">
            ...
            {/* Quick Export History Trigger */}
            {historyCount > 0 && onOpenExport && (
              <div className="w-full pt-1">
                <button
                  type="button"
                  onClick={onOpenExport}
                  className="w-full py-2.5 px-4 rounded-xl bg-slate-900/50 hover:bg-slate-900 border border-white/10 hover:border-violet-500/40 text-xs font-mono text-slate-300 hover:text-white flex items-center justify-between transition-all group"
                  title="Export analysis history as structured JSON"
                >
                  <span className="flex items-center gap-2">
                    <FileJson className="w-4 h-4 text-violet-400 group-hover:text-violet-300" />
                    <span>Export Analysis History (JSON)</span>
                  </span>
